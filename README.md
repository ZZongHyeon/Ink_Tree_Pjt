...
</div>`<th align="center">`🏷️ `<strong>`구분`</strong>``</th>`
`<th align="left">`🔧 `<strong>`기술 스택`</strong>``</th>`
`<th align="center">`📊 `<strong>`버전`</strong>``</th>`
...
`<td>``<strong>`Spring Boot`</strong>``<br/>``<sub>`Spring MVC, Spring Security 통합`</sub>``</td>`
...
`<td>``<strong>`MyBatis`</strong>``<br/>``<sub>`SQL 매핑 프레임워크`</sub>``</td>`
...
`<td>``<strong>`Oracle Database`</strong>``<br/>``<sub>`엔터프라이즈 RDBMS`</sub>``</td>`
...
`<td>``<strong>`Gradle`</strong>``<br/>``<sub>`의존성 관리 및 빌드 자동화`</sub>``</td>`
...
`<td>``<strong>`Embedded Tomcat`</strong>``<br/>``<sub>`Spring Boot 내장 서버`</sub>``</td>`
...
`<td>``<strong>`Git & GitHub`</strong>``<br/>``<sub>`분산 버전 관리 시스템`</sub>``</td>`
...
`<td>``<strong>`Jira & Notion`</strong>``<br/>``<sub>`프로젝트 관리 및 문서화`</sub>``</td>`
...
`<td>``<strong>`Postman`</strong>``<br/>``<sub>`API 개발 및 테스트 플랫폼`</sub>``</td>`
